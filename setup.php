<?php
	...
	setPageTitle("PARLEY! setup");

	if(!isSet($_POST['setup']))
	{
			$license = file_get_contents("./LICENSE");
			$form = <<<EOT
			<h1>PARLEY! - Simple Forum Setup</h1><br />
			<hr /><br />
			<h2>Welcome to PARLEY!</h2><br />
			<br />
			This setup script will help you configure PARLEY! for the first time so you can begin using it.<br />
...
EOT;
		...
		{
			$issue = true;
			addToBody(error("No. mysqli extension must be installed for PARLEY! to work.<br /><br />", true));
		...
		{
			$issue = true;
			addToBody(error("No. json extension must be installed for PARLEY! to work.<br /><br />", true));
		...
		{
			$issue = true;
			addToBody(error("No. mbstring extension must be installed for PARLEY! to work.<br />", true));
		...
		{
			if($problem)
				addToBody("PARLEY! is probably compatible with this server. You should check the errors above to see if you can improve at all.");
			else
				addToBody("PARLEY! is compatible with this server.");
...
EOT;
		...
		{
			addToBody("Your server has one more issues that makes it imcompatible with PARLEY!");
			finishPage();
		}
	}
		...
		if(!isSet($_POST['server_addr']))
		{
			...
			<div style="text-align: left; width: 75%">
				<form method="POST">
					...
					<tr><td>Forum name/Site name:<span class="rightAlign"><input type="text" name="sitename" value="PARLEY!" required/></span></td><td class="finetext">This will show up in the brower's title bar when visiting the home page.</td></tr>
	...
	KEY(`logUserID`)
			...
			if(!file_exists("./data"))
			{
				if(mkdir("./data") === false)
				{
					...
					if(mkdir("./data") === false)
						finishPage(error("Fatal error: Unable to create data directory. Make sure the directory PARLEY! is installed in is writable."));
				}
